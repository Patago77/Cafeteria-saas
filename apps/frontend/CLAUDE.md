...
- **Gotcha real (hydration mismatch — regresión que llegó a producción):** `useConexion` tiene que arrancar SIEMPRE con `useState(true)` (no un lazy initializer que lea `navigator.onLine`). En SSR no existe `navigator`, así que el servidor siempre renderiza `true`; si el cliente lee el valor real de `navigator.onLine` ya en el primer render (el de hidratación), el HTML no coincide con el del servidor y React tira "Hydration failed" en cualquier página que use `EstadoConexion`/`useConexion` (le pasó justo a esto, ver commit que lo revierte). El valor real se corrige recién en el `useEffect`, que solo corre client-side después de hidratar. Como contrapartida, `usePedidos` puede disparar un `api.get()` de más en el primer render aunque arranques offline (inofensivo, se traga con `.catch`) — **no "optimizar" esto sin probar en el navegador real con SSR**, no alcanza con que los tests de Vitest pasen (jsdom no hidrata, no detecta este tipo de mismatch).
